...
- **Core Components**:
    ...
    - **Meeting Scheduler Agent**: Coordinates multi-party email threads, stores conversation context (`MeetingRequest`, `MeetingParticipant`, `MeetingMessage`), proposes new times using hard-coded availability during testing, replies directly in the original email thread, and can now cancel confirmed meetings by reusing the public-booking cancellation workflow. If the sender hasn’t onboarded yet, Cal responds with a provisional signup email and respects the two-message limit before sending the “limit reached” notice.
