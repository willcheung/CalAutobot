...
- **Public Booking Performance**: Booking pages now use eager-loaded SQLAlchemy queries plus a short-lived in-memory cache for `get_availability_for_range`, so repeated date selections reuse computed availability instead of refetching from Google/DB each time.
