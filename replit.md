...
- **Core Components**:
    ...
    - **Meeting Scheduler Agent**: Coordinates multi-party email threads, stores conversation context (`MeetingRequest`, `MeetingParticipant`, `MeetingMessage`), proposes new times using hard-coded availability during testing, replies directly in the original email thread, auto-updates contacts with follow-up counts and touchpoints, and now supports both cancelling and rescheduling confirmed meetings by reusing the public-booking workflows. If the sender hasn’t onboarded yet, Cal responds with a provisional signup email and respects the two-message limit before sending the “limit reached” notice.
...
- **Scheduling Idempotency**: Gmail ingestion skips assistant-sent messages and ignores duplicate `message_id`s before invoking the scheduler, preventing runaway reply loops and duplicate bookings when Pub/Sub replays messages.
- **Contacts Service**: `app/services/contacts.py` centralises contact upserts, label management (including automatic “Booked” tagging), and interaction tracking when the agent sends or receives email.
- **Public Booking Performance**: Booking pages now use eager-loaded SQLAlchemy queries plus a short-lived in-memory cache for `get_availability_for_range`, so repeated date selections reuse computed availability instead of refetching from Google/DB each time. Public bookings also feed the contacts table for CRM-style tracking.
    - **Database Models**: `User`, `Event`, `TextInput`, `EmailAttachment`, `MeetingRequest`, `MeetingParticipant`, `MeetingMessage`, `Contact`, `ContactLabel`, `ContactLabelLink`, `GmailPushState`, and `CalWaitlist` capture user data, extraction history, conversations, contacts, and push state.
...
- **Icons**: Feather Icons.
- **UI/UX Decisions**: Responsive design, consistent iconography, clear user journey (e.g., "Forward Email or Take Screenshot" -> "AI Processes Your Content" -> "Auto-Sync to Google Calendar"), optimized bookings layout, mobile responsiveness with hidden navbar buttons and dedicated mobile sections, plus a compact Contacts table view (sidebar shortcut under Bookings) showing labels, company, role, and last interaction.
