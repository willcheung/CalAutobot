...
class EmailSummary(db.Model):
    """Stores AI-generated summaries and categorizations of emails"""
    ...
    # Categories: 'newsletter', 'promotion', 'personal', 'business', 'urgent', 'scheduling', 'other'

    # Priority and action tracking
    priority_score = db.Column(db.Integer, nullable=True)  # 1-10 scale
    action_required = db.Column(db.Boolean, default=False, nullable=False)
    ...
    # Relationships
    user = db.relationship('User', backref=db.backref('email_summaries', lazy='dynamic'))
    text_input = db.relationship('TextInput', backref=db.backref('email_summary', uselist=False))
...
class SenderPattern(db.Model):
    """Tracks patterns and statistics for email senders per user"""
    ...
    # Pattern tracking
    category = db.Column(db.String(50), nullable=True)  # Most common category
    email_count = db.Column(db.Integer, default=1, nullable=False)
    last_email_date = db.Column(db.DateTime, nullable=True)
    first_email_date = db.Column(db.DateTime, nullable=True)

    # Statistics
    avg_priority_score = db.Column(db.Float, nullable=True)
    total_actions_required = db.Column(db.Integer, default=0, nullable=False)
    ...
    # Relationships
    user = db.relationship('User', backref=db.backref('sender_patterns', lazy='dynamic'))

    # Unique constraint on user + sender_email
    __table_args__ = (
        db.UniqueConstraint('user_id', 'sender_email', name='unique_user_sender'),
    )
    ...
    def to_dict(self):
        return {
            ...
            'last_email_date': self.last_email_date.isoformat() + 'Z' if self.last_email_date else None,
            'first_email_date': self.first_email_date.isoformat() + 'Z' if self.first_email_date else None,
            'avg_priority_score': self.avg_priority_score,
            'total_actions_required': self.total_actions_required,
        }
